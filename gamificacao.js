...
(() => {
  ...
  const ensureStyles = () => {
    ...
    style.textContent = `
      ...
      .gm-xp-toast{position:fixed;left:50%;bottom:calc(92px + env(safe-area-inset-bottom));transform:translateX(-50%) translateY(14px);background:var(--surface);border:1px solid var(--accent);color:var(--text);border-radius:999px;padding:9px 18px;font-size:14px;font-weight:850;z-index:99999;opacity:0;transition:opacity .25s var(--ui-motion,cubic-bezier(.23,1,.32,1)),transform .25s var(--ui-motion,cubic-bezier(.23,1,.32,1));box-shadow:0 10px 26px rgba(0,0,0,.35);display:flex;align-items:center;gap:8px}
